...
def notify(userid, message):
    clients[userid].send('%c%c%s' % (0x81, len(message), message))
...
Sec-WebSocket-Accept: %s\r\n\r\n' % token)
        while True:
            ...
            clients[user_id] = self.connection
            if len(data) == 0:
                continue
...
class websocket_server(threading.Thread):
    ...
    def run(self):
        ...
        # sock.bind(('192.168.3.195', self.port))
        ip = '0.0.0.0'
        # ip = '127.0.0.1'
        print ip + ":" + str(self.port)
